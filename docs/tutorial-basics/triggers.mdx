...
En resumen, `DB::statement` es una herramienta útil para ejecutar consultas SQL directamente en la base de datos, lo que permite realizar operaciones avanzadas que no están disponibles a través de la API de Eloquent.
:::
...
En resumen, la elección entre utilizar una vista 
